...
export default function App() {
  ...
  const [loginPhone, setLoginPhone] = useState("+998 90 ");
  const [loginName, setLoginName] = useState("");
  const [isLoggedIn, setIsLoggedIn] = useState(false);
  const [userName, setUserName] = useState("Abu Bakr");
  const [userPhone, setUserPhone] = useState("+998 90 *** ** **");
  const [userBonus, setUserBonus] = useState(1250);
  ...
  const cartSubtotal = Object.entries(cart).reduce((s,[id,qty])=>{const item=resto?.menu.find(m=>m.id===+id);return s+(item?.price||0)*qty;},0);
  ...
  const openResto = (r) => { setResto(r); setView("rest"); setCart({}); setMenuCat(r.menuCats[0]); setPromoApplied(null); setPromoErr(""); };
  ...
  const addItem = id => {setCart(p=>({...p,[id]:(p[id]||0)+1}));const it=resto?.menu.find(m=>m.id===id);addToast((it?.name||"Taom")+" qo'shildi ✓");};
  ...
  const placeOrder = () => {
    ...
    const items = Object.entries(cart).map(([id,qty])=>({...resto.menu.find(m=>m.id===+id),qty}));
    const etaMins = estDelivery(resto);
    const ord = {
      id:Date.now(), resto:resto.name, restoId:resto.id, restoE:resto.e, restoBg:resto.bg,
      items, subtotal:cartSubtotal, discount:promoDisc, fee:resto.fee, total:cartTotal,
      date:new Date(), status:"Qabul qilindi", stage:0, eta:etaMins,
      addr:activeAddr.addr, pay:payMethod, noCall, courierNote,
      reviewed:false, stageTime:Date.now()
    };
    const token = localStorage.getItem("sf_token");
    const orderPayload = {
      restaurant_id: resto.id, items: Object.entries(cart).map(([id,qty])=>{
        const it=resto.menu.find(m=>m.id===+id);
        return {id:+id,name:it?.name,price:it?.price,qty};
      }),
      subtotal:cartSubtotal, discount:promoDisc, delivery_fee:resto.fee||resto.delivery_fee,
      total:cartTotal, address:activeAddr.addr,
      lat:activeAddr.lat, lon:activeAddr.lon,
      payment_method:payMethod, promo_code:promoApplied||undefined,
    ...
    api.post("/api/orders", orderPayload, token).then(apiOrd => {
      const ordId = apiOrd.id || Date.now();
      setOrders(prev=>[{...ord, id:ordId},...prev]);
      setUserBonus(b=>b+Math.round(cartSubtotal*0.01));
      setCart({}); setPromoApplied(null); setPromoCode(""); setCheckoutOpen(false);
      addToast("Buyurtma qabul qilindi! 🎉");
      setTrackingOrder(ordId);
      setView("tracking");
      simulateOrder(ordId);
    }).catch(()=>{
      ...
      setView("tracking");
      simulateOrder(ord.id);
    });
  };

  const simulateOrder = ordId => {
    let stage = 0;
    const advance = () => {
      stage++;
      if(stage >= ORDER_STAGES.length) return;
      const delay = stage===1?12000:stage===2?18000:20000;
      timerRef.current[ordId] = setTimeout(()=>{
        setOrders(prev=>prev.map(o=>o.id===ordId?{...o,stage,status:ORDER_STAGES[stage],stageTime:Date.now()}:o));
        if(stage===ORDER_STAGES.length-1){
          addToast("Buyurtma yetkazildi! Baholang ⭐");
          setOrders(prev=>prev.map(o=>o.id===ordId?{...o,canReview:true}:o));
        }
        advance();
      },delay);
    };
  ...
  const menuItems = resto ? (menuCat ? resto.menu.filter(m=>m.cat===menuCat) : resto.menu) : [];
  ...
  if(view==="splash") return (
    <div style={{...W,display:"flex",flexDirection:"column",alignItems:"center",justifyContent:"center",minHeight:"100vh"}}>
      ...
      <button onClick={()=>{setAdminOpen(true);setView("main");}} style={{marginTop:16,background:"none",border:"none",fontSize:13,color:"#aaa",cursor:"pointer"}}>Admin panel</button>
    </div>
  );
  ...
  if(view==="auth") return (
    <div style={{...W,padding:"40px 24px"}}>
      ...
      <div style={{display:"flex",background:"#f5f0eb",borderRadius:16,padding:4,marginBottom:24}}>
        {["login","register"].map(t=>(
          <button key={t} onClick={()=>setLoginView(t)} style={{flex:1,padding:"10px",borderRadius:12,border:"none",background:loginView===t?"white":"transparent",fontFamily:"inherit",fontWeight:700,fontSize:14,cursor:"pointer",color:loginView===t?"#1a1a1a":"#888"}}>
            {t==="login"?"Kirish":"Ro'yxatdan o'tish"}
          </button>
        ))}
      </div>
      {loginView==="register"&&<div style={{marginBottom:14}}><label style={{fontSize:13,fontWeight:700,color:"#555",display:"block",marginBottom:6}}>Ismingiz</label><input type="text" value={loginName} onChange={e=>setLoginName(e.target.value)} placeholder="Abu Bakr"/></div>}
      <div style={{marginBottom:14}}><label style={{fontSize:13,fontWeight:700,color:"#555",display:"block",marginBottom:6}}>Telefon raqam</label><input type="tel" value={loginPhone} onChange={e=>setLoginPhone(e.target.value)} placeholder="+998 90 000 00 00"/></div>
      <button className="ob" onClick={()=>{if(loginView==="register"&&loginName)setUserName(loginName);if(loginPhone)setUserPhone(loginPhone);api.post("/api/auth/login", {phone: loginPhone}).then(data => {
        if(data.token){
          localStorage.setItem("sf_token", data.token);
          if(data.user?.name) setUserName(data.user.name);
          if(data.user?.phone) setUserPhone(data.user.phone);
          if(data.user?.bonus_points) setUserBonus(data.user.bonus_points);
        }
        setIsLoggedIn(true);setView("main");addToast("Xush kelibsiz! 👋");
      }).catch(()=>{setIsLoggedIn(true);setView("main");addToast("Xush kelibsiz! 👋");});}} style={{width:"100%",padding:"15px",fontSize:15,borderRadius:16,marginTop:8}}>
        {loginView==="login"?"Kirish →":"Ro'yxatdan o'tish →"}
      </button>
      <button onClick={()=>setView("main")} style={{width:"100%",marginTop:12,padding:"12px",borderRadius:16,border:"none",background:"transparent",fontSize:14,color:"#aaa",cursor:"pointer",fontFamily:"inherit"}}>
        Keyinroq kirish
      </button>
    </div>
  );
          ...
          <div style={{padding:"0 0 100px"}}>
            ...
            <div style={{padding:"16px"}}>
              ...
              <div style={{background:"white",borderRadius:20,padding:"16px",marginBottom:80,boxShadow:"0 4px 16px rgba(0,0,0,.07)"}}>
                ...
                {Object.entries(cart).map(([id,qty])=>{const it=r.menu.find(m=>m.id===+id);return it?(<div key={id} style={{display:"flex",justifyContent:"space-between",marginBottom:6}}><span style={{fontSize:13,color:"#555"}}>{it.name} × {qty}</span><span style={{fontSize:13,fontWeight:700}}>{fmt(it.price*qty)}</span></div>):null;})}
                <div style={{borderTop:"1px solid #f5e6d8",marginTop:10,paddingTop:10}}>
          ...
          <>
            ...
            <div style={{background:"white",borderBottom:"1px solid #f5e6d8",padding:"12px 0 0"}}>
              <div className="hs" style={{padding:"0 16px 12px"}}>
                {r.menuCats.map(c=>(
                  <button key={c} onClick={()=>setMenuCat(c)} style={{flexShrink:0,padding:"8px 16px",borderRadius:20,border:"none",fontFamily:"inherit",fontWeight:700,fontSize:13,cursor:"pointer",transition:"all .15s",background:menuCat===c?P:"#f5f5f5",color:menuCat===c?"white":"#666"}}>
                    {c}
                  </button>
                ))}
              </div>
                  ...
                  return (
                    <div key={item.id} style={{background:"white",borderRadius:16,padding:"12px 14px",display:"flex",alignItems:"center",gap:12,boxShadow:"0 2px 10px rgba(0,0,0,.06)"}}>
                      ...
                      <div style={{flex:1,minWidth:0}}>
                        ...
                        <div style={{fontSize:11,color:"#aaa",marginBottom:6,overflow:"hidden",textOverflow:"ellipsis",whiteSpace:"nowrap"}}>{item.desc}</div>
        ...
        <>
          ...
          <div style={{padding:"16px 16px 100px"}}>
            <div style={{background:"white",borderRadius:20,padding:"20px",marginBottom:14,boxShadow:"0 4px 16px rgba(0,0,0,.07)"}}>
              <div style={{display:"flex",alignItems:"center",gap:14,marginBottom:16}}>
                ...
                <div style={{flex:1}}>
                  {editProfile?(
                    <div>
                      <input type="text" value={userName} onChange={e=>setUserName(e.target.value)} style={{marginBottom:6,fontSize:15,fontWeight:800}} placeholder="Ismingiz"/>
                      <input type="tel" value={userPhone} onChange={e=>setUserPhone(e.target.value)} style={{fontSize:13}} placeholder="Telefon"/>
                    </div>
                  ):(
                    <div>
            ...
            <div style={{background:"white",borderRadius:20,padding:"4px 16px",marginBottom:14,boxShadow:"0 4px 16px rgba(0,0,0,.07)"}}>
              ...
              <div className="pr" onClick={()=>setAdminOpen(true)}><div style={{display:"flex",alignItems:"center",gap:12}}><div style={{width:36,height:36,borderRadius:10,background:"#f5f0eb",display:"flex",alignItems:"center",justifyContent:"center"}}><Settings size={17} color="#888"/></div><span style={{fontWeight:700,fontSize:14,color:"#1a1a1a"}}>Admin panel</span></div><ChevronRight size={18} color="#ddd"/></div>
              <div className="pr" style={{borderBottom:"none"}} onClick={()=>{setIsLoggedIn(false);setView("auth");}}>
